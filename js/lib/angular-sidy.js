...
;(function ( angular, window, document, undefined ) {
	...
	function $elemByClass(classname){
		return angular.element( document.getElementsByClassName( classname ) );
	}
	function $elemBody(){
		return angular.element( document.body );
	}
	function $before($dest, $src){
		var elemSrc,
			elemTmp;
		
		if (angular.isString($src)){
			elemTmp = document.createElement("div");
			elemTmp.innerHTML = $src;
			elemSrc = elemTmp.children;
		}
		else{
			elemSrc = $src;
		}
		
		console.log("elemSrc", elemSrc, "$dest", $dest);
		
		$dest.parent()[0].insertBefore( elemSrc[0], $dest[0] );
		
		return $dest;
	}
	function $css($dest, mCss){
		angular.forEach(mCss, function(val, key){
			$dest.css(key, val);
		});
	...
	var pluginName = "Sidy",
        ...
        hasParentClass = function(element, classname, has) {
            var parent = angular.element(element).parent();
            
            console.log("hasParentClass", parent);
            
            if (parent.prop("tagName") !== "BODY"){
            	has = hasParentClass(parent, classname, parent.hasClass(classname) || has );
            }
	...
	function Fx () {
		...
		this.output = {
			css: {
				opened: angular.extend( {}, elements ),
				closed: angular.extend( {}, elements ),
			},
			inPusher:  false,
			afterNone: false,
		};
	...
	Fx.prototype = {
		...
		slide_overlay: function (position, size, duration, easing) {
			var options = angular.merge({}, this.output),
				p = this._getPosition(position, size, '100%');

			options.css.opened.panel = angular.extend(
				{},
				{visibility: 'visible'},
			...
			options.css.closed.panel = angular.extend(
				{},
				// {visibility: 'visible'},
			...
			options.css.closed.pusher = angular.extend(
				{},
		...
		reveal: function (position, size, duration, easing) {
			...
			options.css.opened.pusher = angular.extend(
				{},
				{visibility: 'visible'},
		...
		scale_down: function (position, size, duration, easing) {
			...
			options.css.opened.container = angular.extend(
				{},
	...
	function Panels (panelClass, Fx) {
		var self = this,
			jqPanel = $elemByClass(panelClass);
		
		angular.forEach(jqPanel, function(elem, key){
			var $p = angular.element(elem),
				optionsFromHTML = {
			...
			if (o.size === 'auto') {
				o.size = (o.position === 'left' || o.position === 'right')? elem.offsetWidth + 'px': elem.offsetHeight + 'px';
			};
	...
	function Sidy ( element, optionsHTML ) {
		this.options = angular.extend( {}, defaults, optionsHTML );
	...
	Sidy.prototype = {
		init: function () {
			var sidy = this,
				contentHTML = $elemByClass( classes.content ).clone().wrap('<div>').parent().html();

			// Build Sidy html template
			$elemByClass(pluginCssClass)
				.html('')
				.removeClass(pluginCssClass)
				.addClass(classes.container)
				.append('<div></div>');

			$elemByClass(classes.container).find("div").addClass(classes.pusher);
			$elemByClass(classes.pusher).html(contentHTML);

			$elemBody().addClass(classes.statusClosed);

			// Panels and add some classes.
			angular.forEach(sidy.panels, function(options, id) {
				var $pusher  = $elemByClass(classes.pusher),
					$content = $elemByClass(classes.content),
					$div = (options.inPusher) ? $content : $pusher;
				
				$before($div, options.html);
			...
			sidy.$buttonsOpen  = $elemByClass(classes.toOpen);
			sidy.$buttonsClose = $elemByClass(classes.toClose);

			// Add event listeners.
			sidy.$buttonsOpen.on(eventtype, function(event){
				var id = angular.element(this).attr(buttonAttr);
		...
		openPanel: function(event, idPanel, options) {
            var sidy = this,
				$container = $elemByClass(classes.container),
				$panel     = $elemById(idPanel),
				$pusher    = $elemByClass(classes.pusher);
            ...
            setTimeout(function() {
            	$css($container, options.css.opened.container)
					.addClass(classes.opened);
				$css($panel, options.css.opened.panel);
				setTimeout(function() {
					$css($pusher, options.css.closed.pusher);
					$css($pusher, options.css.opened.pusher);
				}, 25);
            }, 1);
            ...
            angular.element(document).on(eventtype, function(evt){
                if (! hasParentClass(evt.target, classes.panel)) {
                    sidy.closePanel(idPanel, options);
                    angular.element(this).off();
                };
            });
        ...
        closePanel: function(idPanel, options) {
			var $container = $elemByClass(classes.container),
				$panel     = $elemById(idPanel),
				$pusher    = $elemByClass(classes.pusher);

            $container.removeClass(classes.opened);
            $css($panel.attr('style', ''), options.css.closed.panel);
			$css($pusher.attr('style', ''), options.css.closed.pusher);
			$elemBody()
	...
	var m = angular.module("ngSidy", []);
	
	m.directive("ngSidy", function(){
		return {
			restrict: "A",
			scope: {
				ngSidyScope: "="
